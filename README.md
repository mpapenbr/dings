...
[![Build Status](https://img.shields.io/github/checks-status/mpapenbr/dings/main?color=black&style=for-the-badge&logo=github)][github-actions]
...
```sh
git clone https://github.com/mpapenbr/dings
cd dings
```
...
This section will guide you to setup a fully-functional local copy of `dings`
...
from [`dings's` Makefile][makefile-file] (most of which are
OS-independent)!

### Installing dependencies

To install all dependencies associated with `dings`, run the
command
...
Tests in `dings` are classified as _fast_ and _slow_ - depending
...
| `run` | Run _dings_ | NA |
| `docker-gen` | Create production docker image for _dings_ | docker |
| `docker-debug` | Create debug-friendly docker image for _dings_ | docker |
...
```text
dings_<version>_<target-os>_<architecture>.<extension>
```
...
```sh
dings_v1.2.1_windows_x86_64.exe
```

### Using Docker

To run `dings` in a docker container, read the instructions in
[docker section](./docker).

### Running `dings`

To run dings, use the command
...
`dings` is powered by a template generated using [`cookiecutter-go`][cookiecutter-go-link] which is based on [`go-template`][go-template-link]
