...
a docker containers for _dings_!
...
```bash
make docker-gen IMAGE=dings VERSION=latest
```
...
```bash
make clean-docker IMAGE=dings VERSION=latest
```

## Debug Images

The [dockerfile](./Dockerfile) for _dings_ uses
...
the base image for the final build stage - these images are referred to as _production_
images.
...
images. These images use `golang:1.20` as base for the final
...
```bash
make docker-debug IMAGE=dings-debug VERSION=latest
```
...
```bash
make clean-docker IMAGE=dings-debug VERSION=latest
```
